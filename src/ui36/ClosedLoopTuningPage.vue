<!--
  ...
  The <script setup> block is almost entirely framework-agnostic business logic (reactive state, the
  HostAdapter, the model modules) — it is copied over near-verbatim, only the component imports change
  (local HelpTip/AboutDialog instead of dwc-plugin-runtime's Vue-3-only ones, diagnostics subpath
  instead of the barrel). Only the TEMPLATE differs, translated to Vuetify 2's API (see
  docs/PLAN-dwc36-backport.md §6 for the translation table this follows: v-stepper's structural
  rewrite, item-title -> item-text, density -> dense, variant="outlined" -> outlined, no #append-inner/
  #append slots on v-alert or v-list-item, v-table -> v-simple-table, no "tonal" variant). Keep the two
  in step when changing tuning behaviour — the safety-critical logic (coupledAxesForDriver,
  ensureAxisReady, captureRaw) is identical on both, not reimplemented.
-->
<template>
...
/** Local stand-in for dwc-plugin-runtime's AboutExtraAction (a Vue-3-only package — see ./AboutDialog). */
...
// About dialog (local Vuetify 2 AboutDialog — see ./AboutDialog) wiring.
