<!--
  DWC 3.6 (Vue 2.7 / Vuetify 2) copy of ../ui37/VibrationChart.vue. The drawing logic is identical —
  Chart.js and the canvas API are framework-agnostic — only the template's Vuetify prop/slot names
  differ. Keep the two in step when changing chart behaviour.
-->
<template>
	<v-card class="fill-height">
		<v-card-title class="d-flex align-center py-2">
			...
			Vibration
			<v-spacer />
			<span v-if="capture" class="text-caption text--secondary">{{ capture.rateHz }} Hz{{ capture.overflows > 0 ? `, ${capture.overflows} dropped samples` : "" }}</span>
		</v-card-title>
		<v-card-text>
			<div v-if="!capture" class="text--secondary text-center py-8">Record with vibration enabled to plot the accelerometer trace here.</div>
			<div v-if="summary" class="caption text--secondary mb-2">{{ summary }}</div>
			<v-alert v-if="shortCoverage" type="warning" dense text class="mb-2 caption">
